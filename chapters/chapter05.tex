...
% \begin{table}
%     \myfloatalign
%     \caption{Land et al.~\cite{Land:2012} model parameters with Longobardi et al.~\cite{Longobardi:2020} baseline values.}\label{tab:landmodelparams}
%     \begin{tabular}{l|l|l}
%         \hline
%         \textbf{Parameter} & \textbf{Definition} & \textbf{Value} \\
%         \hline
%         $\Caif$ & $\Ca$ thin filament sensitivity & $\SI{2.1708}{\micro\Molar\tothe{1-1/\ntrpn}}$ \\
%         $\beta_1$ & phenomenological tension length & $\SI{-1.5}{}$ \\
%         \phantom{$\beta_1$} & -dependence scaling factor & \phantom{$\SI{-1.5}{}$} \\
%         $\ntrpn$ & $\Ca$-TnC binding degree of cooperativity & $\SI{2.0}{}$ \\
%         $\kon$ & binding rate of $\Ca$ to TnC & $\SI{0.1}{\per\micro\Molar\per\milli\second}$ \\
