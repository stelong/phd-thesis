...
\noindent
where the second term is equal to $\mathbb{E}^2[Y]2$, while the first term can be rewritten by expressing the integral argument as an integral in $2(D - 1)$ dimensions:
%
\begin{align}
...
\noindent
Finally, additional $D$ matrices of size $N\times D$ are also built starting from matrix $\mathbf{A}$ and each time replacing a different $i$-th column with the $i$-th column from matrix $\mathbf{B}$:
%
\begin{align}
...
\noindent
In total, only $N\times (D + 2)$ model evaluations, corresponding to $f(\mathbf{A})$, $f(\mathbf{B})$ and $f(\mathbf{A}_{\mathbf{B}}^{(i)}),\,i=1,\dots,D$, will be needed to estimate Sobol' indices, compared to the $2\times D\times N^2$ for the brute force approach. parla degli stimatori che hai usato, prima fai vedere come usare quegli integrali e poi vaffanculo con i gpe
...
\begin{align}
    & \mathbf{X} = [X_1,\dots,\,X_d] \in\mathbb{R}^{n\,\times\,d}\,,\qquad Y = f(X_1,\dots,\,X_d) \\
    & S_i = \frac{\mathbb{V}_{X_i}(\mathbb{E}_{\mathbf{X}_{\sim i}}(Y|X_i))}{\mathbb{V}(Y)} \\
    & S_{Ti} = \frac{\mathbb{E}_{\mathbf{X}_{\sim i}}(\mathbb{V}_{X_i}(Y|\mathbf{X}_{\sim i}))}{\mathbb{V}(Y)} \\
