...
\begin{equation}\label{eq:implmeasure}
    I(\mathbf{x})^2 := \frac{(\mathbb{E}[f_{emul}(\mathbf{x})]-z)^2}{\mathbb{V}[\mathbb{E}[f_{emul}(\mathbf{x})]-z]} = \frac{(\mathbb{E}[f_{emul}(\mathbf{x})]-z)^2}{\mathbb{V}[f_{emul}(\mathbf{x})] + \mathbb{V}[d] + \mathbb{V}[e]}
\end{equation}

\noindent
The implausibility measure is then compared against a pre-defined cutoff value to assess whether the corresponding input point will be likely to produce an acceptable match to the real system obervation. Large implausibility measures (above the chosen cutoff) will deem the related points \textit{implausible}, whether points with an implausibility measure below the cutoff value will be deemed \textit{non-implausible}.
%
\begin{equation}
    I(\mathbf{x})
\end{equation}
