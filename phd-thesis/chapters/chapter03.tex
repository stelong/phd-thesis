...
For a given fixed $\Ca$ transient, cubic-Hermite finite element heart mesh and fibre orientation, we can define a multi-scale, non-linear function that maps every set of $8$ input parameters $\mathbf{x}$ to a set of $12$ scalar output LV features $(y_1,\,\dots,\,y_{12})$:
%
...
\noindent
Having available trained $f_{emul,\,j}$ emulators is powerful as they enable performing model exploration, fitting and uncertainty quantification. These tasks in fact normally require a big number of model evaluations, which is prohibitive when the simulator is computationally intensive to be solved.
