...
\begin{figure}[!ht]
    ...
    \caption{The heart and its conduction system. Adapted from~\cite{Guyton:2021}. Copyright $\textcopyright$ 2021 by Elsevier, Inc.}
    \label{fig:my_label0}
\end{figure}
...
\vspace{0.2cm}
The sarcolemma was modelled as an electrical circuit (\textit{equivalent circuit model}), comprised of a capacitor in parallel with resistors that represent ionic currents. The total membrane current was given as the sum of the capacitive current and the ionic current:
%
...
\vspace{0.2cm}\noindent
and for each ion species the generic transmembrane current $I_{\textrm{ion}}$ was described as:
%
\begin{equation}
...
\begin{tabular}{ll}
    ...
    $V_m$ & transmembrane voltage \\
    $E_\textrm{ion}$ & equilibrium potential of the considered ion species
\end{tabular}

\vspace{0.2cm}\noindent
The equilibrium potential for a particular ion at which its electrochemical gradient is balanced can be expressed through the \textit{Nernst equation}:
%
\begin{equation}
...
\vspace{0.5cm}
Since Noble's work, many ionic cell models have been developed to represent the baseline physiology of both atrial, ventricular, and sinoatrial node myocytes~\cite{Corrado:2020}. These many currently available models all share the electric circuit way of modelling the cell membrane~\eqref{eq:membranecurrent}, and mostly differ in single ion channels kinetics representations. \todo{CONTINUE}
...
\noindent
where $I_m$ is the cell membrane current calculated as in equation~\ref{eq:membranecurrent} using an ionic cellular model (Section~\ref{sec:cell_ep_modelling}), and additionally scaled by a factor $\beta_m$ to match the domains' $3$-dimensional nature:
%
\begin{equation}\label{eq:membranecurrentsc}
    I_m = \beta_m\left(C_m\,\der{V_m}{t} + \sum_{\textrm{ion}}I_\textrm{ion}\right)
\end{equation}
...
\begin{align}\label{eq:bidomainmodel}
    & \nabla\cdot(\mathbf{G}_i(\nabla V_m + \nabla \phi_{\text{e}})) = \beta_m\left(C_m\,\der{V_m}{t} + \sum_{\textrm{ion}}I_\textrm{ion}\right) \\
