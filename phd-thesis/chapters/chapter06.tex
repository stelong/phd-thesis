...
\vspace{0.2cm}
As the $\Ca$ transients obtained in the presence of the compound at different concentrations can be encoded by $\dca$, $\ampl$, $\tp$ and $\rtf$ features (Section~\ref{sec:ch6encodingcalciumtransientvariations}), we can visualise how these are varying in a dose-dependent manner. These dose-response curves are shown in Figure~\ref{fig:cafeatsverapamilrespcurve} for verapamil as an example compound.
