...
\begin{remark}{Outline}
    In this chapter, we use the personalised SHAM rat model as a reference model of healthy rat heart contraction to show that it is possible to map pharmacological interventions at the sarcomere level through to whole-organ function. For this purpose, we consider omecamtiv mecarbil drug, which action is well characterised at the cellular level. We finally show that it is possible to go backwards. 
\end{remark}
...
%
%
\subsection{Sarcomere parameter space}\label{sec:ch5sarcomereparameterspace}
We modelled the OM effect at the sarcomere level by altering parameters that are specifically responsible for cross-bridge dynamics, namely $\kxb$, $\nxb$, $\trpnf$ and  $\tref$, introduced in Section~\ref{sec:ch2contractionmodel}. All the other parameters, including the ones regulating $\Ca$ binding to TnC ($\Caif$, $\beta_1$, $\ntrpn$, $\kon$, $\koff$) were kept fixed to either the Land et al.~\cite{Land:2012} model baseline values or to the fitted values from the SHAM rat model (Table \todo{talk about best l2 sham fit parameter and link it here}) when these were available.


%
%
%
\subsection{Mapping sarcomere parameter space to LV function}\label{sec:ch5mappingsarcomereparameterspacetolvfunction}
...
%
%
\subsection{Surrogate mapping}\label{sec:ch5surrogatemapping}
We sampled $4096$ points from a LHD in the $4$-dimensional sarcomere parameter space with initial ranges given as the union of a $\pm\SI{50}{\percent}$ perturbation around reference parameter values and the range of parameter values inferred from \textit{in vitro} force-pCa data of skinned rat myocyte preparations~\cite{Nagy:2015, Kampourakis:2018, Kieu:2019} (see Section~\ref{sec:ch5case2} for details). The full model was run at these points and the successfully completed simulations formed the training dataset ($1189$ points). Univariate GPEs were then trained with a $5$-fold cross-validation to predict each of the $14$ LV output features. The accuracy of each of the resulting $14$ trained GPEs was evaluated using the $R^2$-score regression metric. The resulting mean cross-validation $R^2$ test score was $>0.98$ for all the GPEs.
...
\begin{table}[!ht]
    ...
    \begin{tabularx}{\textwidth}{XXX}
        ...
        \bottomrule
    \end{tabularx}
    \caption{Input parameter ranges used for describing OM sarcomere space.}
