...
\begin{remark}{Outline}
    In this chapter, we use the personalised SHAM rat heart contraction model as a reference model of a healthy rat heart to show that it is possible to quantitatively map pharmacological interventions at the sarcomere level through to the whole-organ function. Moreover, we show that we can infer cell-level pharmacological compounds' effects from whole-organ observations, making the inverse mapping possible. We apply this framework to an example drug, namely omecamtiv mecarbil, which has been proposed as a possible treatment of HF.
